...
class UNet(nn.Module):
    def __init__(self,
        ...
        self.decoder_5 = self._decoder(input_channel=num_channels_down[4],
        ...
        self.conv1 = common3D.conv(input_channel=191,
                                   output_channel=191,
                                   downsample_mode='stride',
                                   kernel_size=3,
                                   stride=2,
                                   ...
                                   group=191,
                                   pad='reflection')
        self.bn1 = common3D.bn(191)
        self.act1 = common3D.act(activate='LeakyReLU')
        self.conv2 = common3D.conv(input_channel=191,
                                   output_channel=191,
                                   kernel_size=3,
                                   bias=True,
                                   group=191,
                                   pad='reflection')
        self.bn2 = common3D.bn(191)
        self.act2 = common3D.act(activate='LeakyReLU')

    def _encoder(self, input_channel, output_channel, kernel_size, bias, pad, activate, downsample_mode):
        encoder = nn.Sequential()

        encoder.add_module('0:downsample', common3D.conv(input_channel=input_channel,
                                                         output_channel=output_channel,
                                                         downsample_mode=downsample_mode,
                                                         ...
                                                         bias=bias,
                                                         pad=pad))
        encoder.add_module('1:bn', common3D.bn(output_channel))
        encoder.add_module('2:act', common3D.act(activate=activate))
        encoder.add_module('3:conv', common3D.conv(input_channel=output_channel,
                                                   output_channel=output_channel,
                                                   kernel_size=kernel_size,
                                                   bias=bias,
                                                   pad=pad))
        encoder.add_module('4:bn', common3D.bn(output_channel))
        encoder.add_module('5:act', common3D.act(activate=activate))
    ...
    def _decoder(self, input_channel, output_channel, kernel_size, bias, pad, activate, need1x1_up):
        ...
        decoder.add_module('0:bn', common3D.bn(num_features=input_channel))
        decoder.add_module('1:conv', common3D.conv(input_channel=input_channel,
                                                   output_channel=output_channel,
                                                   kernel_size=kernel_size,
                                                   bias=bias,
                                                   pad=pad))
        decoder.add_module('2:bn', common3D.bn(num_features=output_channel))
        decoder.add_module('3:act', common3D.act(activate=activate))
        decoder.add_module('4:conv', common3D.conv(input_channel=output_channel,
                                                   output_channel=output_channel,
                                                   kernel_size=kernel_size,
                                                   bias=bias,
                                                   pad=pad))
        decoder.add_module('5:bn', common3D.bn(num_features=output_channel))
        decoder.add_module('6:act', common3D.act(activate=activate))

        if need1x1_up:
            decoder.add_module('7:conv', common3D.conv(input_channel=output_channel,
                                                       output_channel=output_channel,
                                                       kernel_size=1,
                                                       bias=bias,
                                                       pad=pad))
            decoder.add_module('8:bn', common3D.bn(num_features=output_channel))
            decoder.add_module('9:act', common3D.act(activate=activate))
    ...
    def _skip(self, input_channel, output_channel, kernel_size, bias, pad, activate):
        ...
        skip_connect.add_module('0:conv', common3D.conv(input_channel=input_channel,
                                                        output_channel=output_channel,
                                                        kernel_size=kernel_size,
                                                        bias=bias,
                                                        pad=pad))
        skip_connect.add_module('1:bn', common3D.bn(num_features=output_channel))
        skip_connect.add_module('2:act', common3D.act(activate=activate))
    ...
    def _output(self, input_channel, output_channel, bias, pad, need_sigmoid):
        ...
        layer.add_module('0:conv', common3D.conv(input_channel=input_channel,
                                                 output_channel=output_channel,
                                                 kernel_size=1,
                                                 bias=bias,
                                                 pad=pad))
    ...
    def forward(self, data):
        ...
        enc_1 = self.conv1(data)
        enc_1 = self.bn1(enc_1)
        enc_1 = self.act1(enc_1)
        enc_1 = self.conv2(enc_1)
        enc_1 = self.bn2(enc_1)
        enc_1 = self.act2(enc_1)
